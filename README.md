...
<p align="center">
  ...
  <img alt="Version 0.2.0 alpha 1 build 19" src="https://img.shields.io/badge/version-0.2.0--alpha.1%20(19)-E3A008">
</p>
...
- Play compatible 2D Scenes live; render unsupported Scene features to a validated 20-second, 30 FPS MP4 cache when the external renderer and user-provided engine assets are available. Build 19 admits only immutable Scene cache generations whose sidecar, size, and SHA-256 match, falls back to an older verified generation, and deduplicates recovery if cached playback fails on several displays. The queue runs at most two distinct external renders, preserves FIFO order, and starts each render timeout only after that job receives a permit; balanced/high renders retry once at low quality after a timeout. Authored sound resolves from `scene.pkg`, the unpacked project, then engine assets without permitting path or symlink escape.
...
The current source milestone is **v0.2.0-alpha.1, build 19**. Prebuilt artifacts, when published, are available from [GitHub Releases](https://github.com/LamPPKK/wallpaper-player-mac/releases).
