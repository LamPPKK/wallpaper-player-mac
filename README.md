...
Compatibility probe version 23 recognizes Lively folder dropdowns as fully mapped now that filtered multi-file add, selection, and private-copy deletion are implemented; previously imported projects carrying the legacy limitation are re-probed automatically. It retains probe 22's detection of reachable particle emitters, initializers, operators, renderers, child systems, and collection forms that the bundled Scene renderer would otherwise accept but silently omit. Those Scenes are reported as **Limited** with exact particle-module reasons instead of **Full Cached** merely because preflight emitted one frame. Probe 21's explicit-font renderer routing and the earlier staging-only Web/Lively normalization, remote-runtime classification, and bounded dependency discovery rules remain in force. Discovery examines at most 10,000 project entries, sorts and stages at most 64 eligible candidates, records a path-free truncation diagnostic, and still loads the page. At playback time, AVFoundation-compatible local audio and video remain direct sources; FFmpeg conversion is reserved for sources WebKit cannot play.
