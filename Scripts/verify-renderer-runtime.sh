...
while IFS= read -r file_path; do
  ...
  while IFS= read -r dependency; do
    ...
    case "$dependency" in
      ...
      @executable_path/lib/*)
        ...
        case "$relative" in
          ""|/*|.|..|*/./*|*/../*|*/.|*/..)
            printf '%s\n' "Renderer dependency escapes its runtime: $file_path -> $dependency" >&2
            exit 1
            ;;
        esac
        if [ ! -e "$RUNTIME/lib/$relative" ] && [ ! -L "$RUNTIME/lib/$relative" ]; then
          printf '%s\n' "Renderer dependency is missing: $file_path -> $dependency" >&2
          exit 1
        fi
        ;;
      /opt/homebrew/*|/usr/local/*)
        printf '%s\n' "Renderer contains a Homebrew-only dependency: $file_path -> $dependency" >&2
        exit 1
        ;;
      @rpath/*|@loader_path/*)
        printf '%s\n' "Renderer contains an unresolved dependency: $file_path -> $dependency" >&2
        exit 1
        ;;
      *)
        printf '%s\n' "Renderer contains a non-system external dependency: $file_path -> $dependency" >&2
        exit 1
        ;;
    esac
  ...
  if [ "$rpath_count" -ne 1 ] || [ "$rpaths" != "@executable_path/lib/" ]; then
    printf '%s\n' "Renderer Mach-O must contain exactly one @executable_path/lib/ LC_RPATH: $file_path" >&2
    exit 1
  fi
