...
final class ScreenSaverFeatureTests: XCTestCase {
    ...
    func testScreenSaverKeepsStillFallbackVisibleBehindVideoPlayback() throws {
        ...
        XCTAssertTrue(source.contains("AVURLAssetReferenceRestrictionsKey"))
        XCTAssertTrue(source.contains("AVAssetReferenceRestrictionForbidAll"))
        XCTAssertTrue(source.contains("AVURLAssetShouldSupportAliasDataReferencesKey: @NO"))
        XCTAssertTrue(source.contains("[AVPlayerItem playerItemWithAsset:asset]"))
        XCTAssertFalse(source.contains("[AVPlayerItem playerItemWithURL:url]"))
